...
-- These SECURITY DEFINER helpers are narrowly scoped and avoid recursive RLS checks.
create or replace function public.is_group_member_for_user(target_group_id uuid, target_user_id uuid)
...
returns boolean language sql stable security definer set search_path = public as $$
  select public.is_group_member_for_user(target_group_id, auth.uid());
$$;
create or replace function public.is_group_admin(target_group_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.group_members gm where gm.group_id = target_group_id and gm.user_id = auth.uid() and gm.role = 'admin')
      or exists (select 1 from public.groups g where g.id = target_group_id and g.owner_id = auth.uid());
$$;

revoke all on function public.is_group_member_for_user(uuid, uuid) from public;
grant execute on function public.is_group_member_for_user(uuid, uuid) to authenticated;
revoke all on function public.is_group_member(uuid) from public;
grant execute on function public.is_group_member(uuid) to authenticated;
revoke all on function public.is_group_admin(uuid) from public;
grant execute on function public.is_group_admin(uuid) to authenticated;

-- Database-enforced shared-expense integrity. The deferred constraint trigger allows
-- the parent expense and participant rows to be inserted in one transaction.
create or replace function public.validate_shared_expense_integrity(target_expense_id uuid)
returns void language plpgsql security definer set search_path = public as $$
declare
  expense_group uuid;
  expense_payer uuid;
  expense_total numeric(12,2);
  participant_total numeric(12,2);
  participant_count integer;
  unauthorized_count integer;
begin
  select group_id, paid_by_user_id, total_amount into expense_group, expense_payer, expense_total
  from public.shared_expenses where id = target_expense_id;
  if not found then return; end if;

  if not public.is_group_member_for_user(expense_group, expense_payer) then
    raise exception 'The shared expense payer must be a member of its group';
  end if;

  select count(*), coalesce(sum(amount_owed), 0), count(*) filter (where not public.is_group_member_for_user(expense_group, user_id))
    into participant_count, participant_total, unauthorized_count
  from public.shared_expense_participants where shared_expense_id = target_expense_id;

  if unauthorized_count > 0 then
    raise exception 'Every shared expense participant must be a member of its group';
  end if;
  if participant_count = 0 or participant_total <> expense_total then
    raise exception 'Participant amounts must sum exactly to the shared expense total';
  end if;
end;
$$;
...
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if not public.is_group_member_for_user(new.group_id, new.from_user_id)
     or not public.is_group_member_for_user(new.group_id, new.to_user_id) then
    raise exception 'Settlement users must be members of its group';
  end if;
  return new;
end;
$$;
...
create constraint trigger shared_expenses_validate_integrity after insert or update on public.shared_expenses deferrable initially deferred for each row execute function public.validate_shared_expense_integrity(id);
drop trigger if exists shared_expense_participants_validate_integrity on public.shared_expense_participants;
create constraint trigger shared_expense_participants_validate_integrity after insert or update or delete on public.shared_expense_participants deferrable initially deferred for each row execute function public.validate_shared_expense_integrity(coalesce(new.shared_expense_id, old.shared_expense_id));
...
create policy shared_expenses_member_insert on public.shared_expenses for insert to authenticated with check (public.is_group_member(group_id) and public.is_group_member_for_user(group_id, paid_by_user_id) and created_by_user_id = auth.uid());
drop policy if exists shared_expenses_member_update on public.shared_expenses;
create policy shared_expenses_member_update on public.shared_expenses for update to authenticated using (public.is_group_member(group_id)) with check (public.is_group_member(group_id) and public.is_group_member_for_user(group_id, paid_by_user_id) and created_by_user_id = auth.uid());
...
create policy shared_participants_member_insert on public.shared_expense_participants for insert to authenticated with check (exists (select 1 from public.shared_expenses se where se.id = shared_expense_id and public.is_group_member(se.group_id) and public.is_group_member_for_user(se.group_id, user_id)));
...
-- Settlements remain scoped to group membership; both endpoints are validated.
...
create policy settlements_member_insert on public.settlements for insert to authenticated with check (public.is_group_member(group_id) and public.is_group_member_for_user(group_id, from_user_id) and public.is_group_member_for_user(group_id, to_user_id) and created_by_user_id = auth.uid());
